<template>
  <section>
    ...
    <b-sidebar type="is-light" right fullheight :open.sync="showShortcutHelp">
      <div class="content p-1">
        <h3 class="subtitle">Supported keyboard shortcuts</h3>
        <p>Key combos are zero or more modifiers (<code>ctrl</code>, <code>shift</code>, etc.) plus one regular key
          (letter keys, number keys, arrow keys, etc), all joined with <code>+</code> signs (for keys to be pressed at the
          same time) or spaces (for keys to be pressed one after another.</p>
        <p><strong>Note that you have to use <code>+</code> rather than <code>-</code> to signify pressing keys at the
            same time. Example: <code>ctrl+b</code> will work but <code>ctrl-b</code> will not.</strong></p>
        ...
        <p>The following modifiers are available: <code>⇑</code>, <code>shift</code>, <code>option</code>, <code>⌥</code>,
          <code>alt</code>, <code>ctrl</code>, <code>control</code>, <code>command</code>, and <code>⌘</code>.
        </p>
        <p>In addition to regular letters, numbers, and punctuation, the following special keys can be used for shortcuts:
          <code>backspace</code>, <code>tab</code>, <code>clear</code>, <code>enter</code>, <code>return</code>,
          <code>esc</code>, <code>escape</code>, <code>space</code>, <code>up</code>, <code>down</code>,
          <code>left</code>, <code>right</code>, <code>home</code>, <code>end</code>, <code>pageup</code>,
          <code>pagedown</code>, <code>del</code>, <code>delete</code> and <code>f1</code> through <code>f19</code>.
        </p>
      </div>
    </b-sidebar>

    <b-sidebar type="is-light" right fullheight :open.sync="showBehaviorHelp">
      <div class="content p-1">
        <h3 class="subtitle">Behaviors</h3>
        <p>Here is a <a
            href="https://github.com/mikecrittenden/shortkeys/wiki/How-To-Use-Shortkeys#full-list-of-commands">full list
            of commands</a> with explanations of what each one does.</p>
        <p>Note that Shortkeys makes some behaviors are available in the browser's built in Keyboard Shortcuts UI. Here's
          some <a
            href="https://github.com/mikecrittenden/shortkeys/wiki/FAQs-and-Troubleshooting#Do_I_use_the_browsers_Keyboard_Shortcuts_settings_or_the_Shortkeys_options_page">more
            info about that</a>.</p>
      </div>
    </b-sidebar>
    ...
    <b-tabs v-model="activeTab" type="is-toggle" expanded>
      <b-tab-item label="Shortcuts">
        <b-table :data="keys" ref="table" detailed detail-key="key" :show-detail-icon="showDetailIcon">
          ...
          <template slot="detail" slot-scope="props">
            <b-message title="Try the browser's keyboard shortcut settings" type="is-info"
              v-show="isBuiltIn(props.row.action)" aria-close-label="Close message">
              This action is supported from the browser's keyboard shortcuts settings, which
              will allow it to run on the new tab page and when the address bar is focused, etc. The downside is that the
              browser
              is more restrictive about which shortcuts are supported, and you can't enable or disable on certain websites
              or
              when typing in form fields.<br /><br />To try it, <strong>go back to the extensions page, click the menu
                icon
            ...
            <div class="columns">
              <div class="column">
                ...
                <b-switch v-model="props.row.smoothScrolling"
                ...
                <b-switch v-model="props.row.currentWindow"
                  v-show="props.row.action === 'gototab' || props.row.action === 'gototabbytitle'">
                  Search in current window only
                </b-switch>

                <b-field label="Bookmark"
                  v-show="props.row.action === 'openbookmark' || props.row.action === 'openbookmarknewtab' || props.row.action === 'openbookmarkbackgroundtab' || props.row.action === 'openbookmarkbackgroundtabandclose'">
                ...
                <b-field v-show="props.row.action === 'gototab'">
                  <template slot="label">
                    URL to match (<a target="_blank"
                      href="https://developer.chrome.com/extensions/match_patterns">Examples</a>)
                  </template>
                ...
                <b-field label="Button selector (example: #troop_confirm_go or .button_class_123)"
                  v-show="props.row.action === 'buttonnexttab'">
                  <b-input type="text" v-model="props.row.button"></b-input>
                </b-field>

                <b-field label="App ID to open (Can be found on extensions management page)"
                  v-show="props.row.action === 'openapp'">
              ...
              <div class="column">
                ...
                <b-field>
                  <b-input type="textarea" v-show="props.row.blacklist && props.row.blacklist != 'false'"
                    v-model="props.row.sites" />
                </b-field>
              </div>
            </div>
          </template>
