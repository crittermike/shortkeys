...
let selectTab = direction => {
  ...
  browser.tabs.query(searchParam).then(function(tabs) {
    ...
    browser.tabs
      ...
      .then(function(currentTabInArray) {
...
let isAllowedSite = function(keySetting, url) {
  ...
  let allowed =
    keySetting.blacklist === true || keySetting.blacklist === "true";
...
let handleAction = (action, request = {}) => {
  ...
  if (action === "cleardownloads") {
    ...
    browser.tabs
      .query({ currentWindow: true, pinned: false, active: false })
      .then(function(tabs) {
        let ids = [];
      ...
      .then(function(tabs) {
        ...
        browser.tabs
          ...
          .then(function(tabs) {
            ...
            tabs.forEach(function(tab) {
              if (
                (action === "closelefttabs" && tab.index < currentTabIndex) ||
    ...
    chrome.downloads.search(
      { orderBy: ["-startTime"], state: "complete" },
      downloads => {
    ...
    browser.tabs.executeScript(null, {
      code:
        'window.scrollBy({left: 0, top: -50, behavior: "' +
        smoothScrolling +
        '"})'
    });
    ...
    browser.tabs.executeScript(null, {
      code:
        'window.scrollBy({left: 0, top: -500, behavior: "' +
        smoothScrolling +
        '"})'
    });
    ...
    browser.tabs.executeScript(null, {
      code:
        'window.scrollBy({left: 0, top: -window.innerHeight, behavior: "' +
        smoothScrolling +
        '"})'
    });
    ...
    browser.tabs.executeScript(null, {
      code:
        'window.scrollBy({left: -50, top: 0, behavior: "' +
        smoothScrolling +
        '"})'
    });
    ...
    browser.tabs.executeScript(null, {
      code:
        'window.scrollBy({left: -500, top: 0, behavior: "' +
        smoothScrolling +
        '"})'
    });
    ...
    action === "openbookmarkbackgroundtab" ||
    action === "openbookmarkbackgroundtabandclose"
  ) {
    browser.bookmarks.search({ title: request.bookmark }).then(function(nodes) {
      ...
      if (action === "openbookmark") {
        ...
        browser.tabs
          .create({ url: decodeURI(openNode.url), active: false })
