...
Shortkeys is a cross-browser extension that lets users define custom keyboard shortcuts for browser actions (scrolling, tab management, navigation, running custom JavaScript, video controls, page scripts, and more). It has 200K+ Chrome users. Available for Chrome, Firefox, Edge, and Opera.
...
- **Testing**: Vitest with 405+ tests across 14 test files
...
│   │   ├── App.vue            # Main options UI (~1800 lines)
│   │   ├── index.html
│   │   └── main.ts
...
│       └── main.ts
├── actions/
│   ├── action-handlers.ts     # Map-based action registry (96+ browser actions)
...
│   ├── vim.ts, productivity.ts, youtube.ts, etc.
└── utils/
    ├── actions-registry.ts    # Action definitions, 11 categories, metadata
    ...
    ├── js-snippets.ts         # 25 curated JS snippets (also registered as actions)
    ├── shortcut-conflicts.ts  # Platform-aware browser default conflict detection
    ├── storage.ts             # Cloud sync abstraction (sync → local fallback)
...
├── index.html                 # Community landing page with pack browser
...
└── catalog.json               # Generated from packs (npm run build:site)

scripts/
└── build-catalog.ts           # Generates catalog.json from pack data

tests/                         # 14 test files, 405+ tests
```
